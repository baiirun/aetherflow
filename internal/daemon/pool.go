...
func (p *Pool) spawn(ctx context.Context, task Task) {
	...
	prompt, err := RenderPrompt(p.config.PromptDir, role, task.ID, p.config.Solo, p.config.Runtime)
	if err != nil {
...
func (p *Pool) respawn(taskID string, role Role) {
	...
	prompt, err := RenderPrompt(p.config.PromptDir, role, taskID, p.config.Solo, p.config.Runtime)
	if err != nil {
