...
Launch parallel Task agents to build structured documentation:

```
Task(subagent_type="general-purpose", prompt="You are a solution extractor. Analyze the git history and current code to extract a structured solution document.

1. Run `git log --oneline $(git merge-base HEAD main)..HEAD` to see commits
2. Run `git diff $(git merge-base HEAD main)..HEAD` to see all changes
3. Identify: investigation steps, root cause, working solution with key code patterns
4. Return a structured document with: Problem, Investigation, Root Cause, Solution, Key Code Patterns")

Task(subagent_type="general-purpose", prompt="You are a documentation finder. Search the project's docs/solutions/ directory for related documentation.

1. List existing docs: search for files in docs/solutions/
2. Read relevant ones that relate to the current changes
3. Identify cross-references and related solutions
4. Return a list of related docs with brief summaries of how they connect")

Task(subagent_type="general-purpose", prompt="You are a prevention strategist. Analyze the changes to develop prevention strategies.

1. Run `git diff $(git merge-base HEAD main)..HEAD` to see all changes
2. Identify: what could prevent similar issues in the future
3. Suggest: test cases, assertions, monitoring, or patterns that would catch this earlier
4. Return prevention strategies as a prioritized list")
```
