# Architecture Decision Records

ADRs record accepted choices that are costly to reverse, surprising without
context, and the result of a real trade-off.

| ADR | Decision |
| --- | --- |
| [0001](0001-keep-pi-as-a-headless-subprocess.md) | Keep Pi as a headless subprocess |
| [0002](0002-model-session-association-as-a-discriminated-state.md) | Model Session Association as a discriminated state |
| [0003](0003-own-the-session-event-cursor.md) | Own the Session Event cursor |
| [0004](0004-separate-session-discovery-from-session-runtime.md) | Separate Session discovery from Session runtime |
| [0005](0005-externalize-attachment-bytes.md) | Externalize Attachment bytes |
| [0006](0006-ship-the-daemon-with-the-desktop.md) | Ship the daemon with the desktop |

When a decision changes, add a new ADR and mark the old record as superseded.
