...
| --- | --- | --- | --- | --- |
| Session identity, Agent identity, association, Pi configuration | Session actor state in Rivet | `SessionId` actor key | Actor sleep and daemon restart | The physical Rivet actor ID is not domain identity. |
...
Session listing reads only the Session Directory. Opening a transcript pages
through one Session actor's event log from sequence zero, downloads referenced
Attachments, hydrates those records for display, and derives conversation
messages and tool groups in memory.
