...
| Daemon | Local HTTP API responds to status and shutdown | `go test ./internal/daemon/... -run TestDaemon` | covered | 2025-02-06 |
...
| Control Center | Session detail launches Opencode through the supported attach flow using recorded session metadata | `cd macos/ControlCenter && swift test --filter SessionHandoffLauncherTests` | covered | 2026-03-15 |
